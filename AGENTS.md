...
- **Implemented:** Local-native named Pi profiles capture a concrete command and account root so launches and resumes cannot drift to another account; worktree and folder creation can select these profiles alongside built-in Pi and refuses them for nonlocal targets. See [`docs/reference/pi-terminal-session-ownership.md`](docs/reference/pi-terminal-session-ownership.md#named-launch-profiles).
