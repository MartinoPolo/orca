...
export function getPiAgentStatusExtensionSource(kind: PiAgentKind = 'pi'): string {
  ...
  return [
    ...
    'let activeDelivery: Promise<void> = Promise.resolve()',
    'let activePostController: AbortController | null = null',
    ...(kind === 'pi' ? ['let previousDelivery: Promise<void> | null = null'] : []),
    ...(kind === 'pi' ? ['let piUiPromptDepth = 0', 'let piTurnInFlight = false'] : []),
    ...modelMetadataSourceLines,
    '',
    ...sessionMetadataSourceLines,
    'function updateEventSessionMetadata(ctx: unknown): void {',
    '  const sessionManager = (ctx as { sessionManager?: unknown } | null)?.sessionManager',
    '  if (!sessionManager) {',
    '    updateModelMetadata(ctx)',
    '    return',
    '  }',
    ...
    '  ompRuntime: boolean',
    '): Promise<void> {',
    '  const delivery = deliverPostOnce(hookEventName, extra, metadata, ompRuntime)',
    '  activeDelivery = delivery.catch(() => {})',
    '  return delivery',
    '}',
    '',
    'async function deliverPostOnce(',
    '  hookEventName: string,',
    '  extra: Record<string, unknown>,',
    '  metadata: Record<string, unknown>,',
    '  ompRuntime: boolean',
    '): Promise<void> {',
    ...(kind === 'pi'
      ? ['  if (previousDelivery) await previousDelivery', '  if (disposed) return']
      : []),
    ...
    '  activePostController = controller',
