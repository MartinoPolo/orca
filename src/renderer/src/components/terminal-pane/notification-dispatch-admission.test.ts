...
describe('dispatchTerminalNotification admission', () => {
  ...
  it('delivers a synthetic failure without changing terminal unread', () => {
    dispatchTerminalNotification('wt-primary', {
      source: 'agent-task-complete',
      terminalTitle: 'codex',
      paneKey,
    ...
    expect(window.api.notifications.dispatch).toHaveBeenCalledWith(
      expect.objectContaining({ desktopOnly: true, soundCategory: 'failed' })
    )
    expect(mockState.markWorktreeUnread).not.toHaveBeenCalled()
    expect(mockState.markAgentCompletionPaneUnread).not.toHaveBeenCalled()
    expect(mockState.markTerminalTabUnread).not.toHaveBeenCalled()
  ...
  it.each(['waiting', 'blocked'] as const)(
    'still rejects stale %s attention without a live surface',
    (agentState) => {
      mockState.ptyIdsByTabId = {}
      mockState.agentStatusByPaneKey[paneKey] = makeAgentStatus(paneKey, {
        state: agentState,
