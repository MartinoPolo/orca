...
- **Spinner** — working
- **Red question mark** — waiting on you (permission / needs input)
- **Red dot** — blocked
- **Emerald check** (dashboard) or **emerald dot** (sidebar) — done, not necessarily successful
- **Violet dot** — interrupted or an explicitly reported failure
- **Gray dot** — idle
...
Inline workspace rows identify the exact session that needs attention. The background shows factual state, while an orange outline and dot mean the session is unread. Reading a waiting or blocked session removes only the unread signal; its status remains until the agent reports a new state.

Open **Activity** to use the attention queue. **Attention** includes unresolved waiting or blocked sessions, unread completions or interruptions, explicit failures, and sessions saved for later; **All** shows the remainder. Within each status group, priority runs from P5 (highest) to P1, then unread and oldest outstanding attention.

Right-click a session row to assign P1–P5, add or remove a colored saved marker, or mark the session read/unread. Priority and saved markers follow the provider session across restart and supported resume; they are unavailable until the provider reports a stable session identity.
